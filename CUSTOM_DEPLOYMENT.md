...
本文档对应正式版本 `custom-v0.2.5`。这是 Fourgetu Remnawave 自定义发行版，不是官方 `main` 的原版发行版。

## 版本清单

| 组件       | Git tag         | 产物                                                                  |
| ---------- | --------------- | --------------------------------------------------------------------- |
| Backend    | `custom-v0.2.5` | `ghcr.io/fourgetu/remnawave-backend:custom-v0.2.5`                    |
| Node       | `custom-v0.2.1` | `ghcr.io/fourgetu/remnawave-node:custom-v0.2.1`                       |
| Frontend   | `custom-v0.2.5` | GitHub Release asset `remnawave-frontend.zip`；已内嵌到 Backend image |
| Panel/docs | `custom-v0.2.5` | 本文档、Compose 模板和部署说明                                        |
...
```bash
curl -fsSL https://github.com/Fourgetu/Remnawave/releases/download/custom-v0.2.5/install-panel.sh | bash
```
...
curl -fL -o docker-compose.custom.yml \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.5/docker-compose.custom.yml
curl -fL -o .env.example \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.5/.env.custom.example
cp .env.example .env
```
...
```bash
curl -fsSL https://github.com/Fourgetu/Remnawave/releases/download/custom-v0.2.5/install-node.sh | bash
```
...
curl -fL -o docker-compose.custom-node.yml \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.5/docker-compose.custom-node.yml
curl -fL -o .env.node.example \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.5/.env.custom-node.example
...
- 当前 `main` 不包含这些魔改；本部署文档只适用于同一 `custom-v0.2.5` 版本清单。
