...
本文档对应正式版本 `custom-v0.2.12`。这是 Fourgetu Remnawave 自定义发行版，不是官方 `main` 的原版发行版。

## 版本清单

| 组件       | Git tag         | 产物                                                                  |
| ---------- | --------------- | --------------------------------------------------------------------- |
| Backend    | `custom-v0.2.12` | `ghcr.io/fourgetu/remnawave-backend:custom-v0.2.12`                  |
| Node       | `custom-v0.2.9` | `ghcr.io/fourgetu/remnawave-node:custom-v0.2.9`                       |
| Frontend   | `custom-v0.2.12` | GitHub Release asset `remnawave-frontend.zip`；已内嵌到 Backend image |
| Panel/docs | `custom-v0.2.12` | 本文档、Compose 模板和部署说明                                       |

Backend image 会在构建时下载同 tag 的 Frontend release asset，因此部署时不需要额外运行 Frontend 容器。

本版为 Mixed 入站新增 SOCKS5 / HTTP 单节点代理分享链接快捷复制。使用已保存的凭据；未保存认证修改时禁用复制；noauth 模式须确认开放代理风险。该功能不生成 Remnawave 用户订阅，Mixed 仍不会自动进入普通用户订阅。Backend 源码、数据库结构和 Node 均未更改。Panel 使用 `custom-v0.2.12`，Node 继续使用 `custom-v0.2.9`。
...
```bash
curl -fsSL https://github.com/Fourgetu/Remnawave/releases/download/custom-v0.2.12/install-panel.sh | bash
```
...
curl -fL -o docker-compose.custom.yml \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.12/docker-compose.custom.yml
curl -fL -o .env.example \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.12/.env.custom.example
cp .env.example .env
```
...
```bash
curl -fsSL https://github.com/Fourgetu/Remnawave/releases/download/custom-v0.2.12/install-node.sh | bash
```
...
curl -fL -o docker-compose.custom-node.yml \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.12/docker-compose.custom-node.yml
curl -fL -o .env.node.example \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.2.12/.env.custom-node.example
...
按本次版本清单更新镜像：Panel 使用 `custom-v0.2.12`，Node 有意继续使用 `custom-v0.2.9`。本版不新增数据库 migration，已有 migration 仍按项目原机制处理；升级前必须先备份 PostgreSQL。然后执行：
...
- 当前 `main` 不包含这些魔改；本部署文档只适用于上述 `custom-v0.2.12` 版本清单。
