...
本文档对应候选版本 `custom-v0.1.0-rc.1`。这是基于 `dev-dual-core` 的实验性发布，不是当前官方 `main` 的稳定版。

## 版本清单

| 组件 | Git tag | 产物 |
|---|---|---|
| Backend | `custom-v0.1.0-rc.1` | `ghcr.io/fourgetu/remnawave-backend:custom-v0.1.0-rc.1` |
| Node | `custom-v0.1.0-rc.1` | `ghcr.io/fourgetu/remnawave-node:custom-v0.1.0-rc.1` |
| Frontend | `custom-v0.1.0-rc.1` | GitHub Release asset `remnawave-frontend.zip`；已内嵌到 Backend image |
| Panel/docs | `custom-v0.1.0-rc.1` | 本文档、Compose 模板和部署说明 |
...
curl -fL -o docker-compose.custom.yml \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.1.0-rc.1/docker-compose.custom.yml
curl -fL -o .env.example \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.1.0-rc.1/.env.custom.example
cp .env.example .env
```
...
curl -fL -o docker-compose.custom-node.yml \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.1.0-rc.1/docker-compose.custom-node.yml
curl -fL -o .env.node.example \
  https://raw.githubusercontent.com/Fourgetu/Remnawave/custom-v0.1.0-rc.1/.env.custom-node.example
...
- 这是 RC 版本。当前实现仍需要在真实 NAT VPS/LXC、UDP、nftables、GOST reload、双 core 和客户端组合上做完整验证。
- 当前 `main` 不包含这些魔改；本部署文档只适用于同一 `custom-v0.1.0-rc.1` 版本清单。
